...
out vec2 out_position; // Record this one

void issue(vec4 pos)
{
    out_position = (mvp * pos).xy;
...
void main()
{
    ...
    for (int i = 0; i < 48; ++i)
    {
        vec2 offset = texelFetch(sphere_triangles, i*3+0).xy * scale;
        issue(pos+vec4(offset, 0., 0.));

        offset = texelFetch(sphere_triangles, i*3+1).xy * scale;
        issue(pos+vec4(offset, 0., 0.));

        offset = texelFetch(sphere_triangles, i*3+2).xy * scale;
        issue(pos+vec4(offset, 0., 0.));
...
class Example(QtOpenGL.QGLWidget):
    ...
    def initializeGL(self):
        ...
        varyings = ctypes.create_string_buffer('out_position')
        varyings_pp = POINTER(POINTER(c_char))(ctypes.cast(varyings, POINTER(c_char)))
        glTransformFeedbackVaryings(program.programId(), 1, varyings_pp, GL_SEPARATE_ATTRIBS)
        if not program.link():
          print(program.log())
        ...
        glBufferData(GL_ARRAY_BUFFER, 9*350*144*2*4, c_void_p(0), GL_STATIC_DRAW)
        glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 2*4, c_void_p(0))
        glEnableVertexAttribArray(0)
        glBindVertexArray(0)
        glBindBuffer(GL_ARRAY_BUFFER, 0)

        self.feedback_program = QGLShaderProgram()
        self.feedback_program.addShaderFromSourceCode(QGLShader.Vertex, vertex_shader_source)
        self.feedback_program.addShaderFromSourceCode(QGLShader.Fragment, fragment_shader_source)
        if not self.feedback_program.link():
            print('Feedback shader failed to link!')
    ...
    def paintGL(self):
        ...
        if self.feedback_generated:
            self.feedback_program.bind()
            glBindVertexArray(self.feedback_vao)
            glDrawArrays(GL_TRIANGLES, 0, self.num_primitives*3)
            glBindVertexArray(0)
            self.feedback_program.release()
        else:
            self.program.bind()
            ...
            self.num_primitives = glGetQueryObjectuiv(qid, GL_QUERY_RESULT)
            glBindVertexArray(0)
